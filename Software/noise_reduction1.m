...
%% 读取文件
raw_file = '去噪测试音频样本1.m4a';
noise_file = '去噪测试音频样本1_留噪.m4a';
voice_file = '去噪测试音频样本1_降噪.m4a';
...
sum_fft_results2= zeros(46,window_size);
sum_fft_results3= zeros(46,window_size);
real_result = zeros(1024, 1);           % 用于存储每次逆FFT处理后的实部结果
comparison_result = zeros(46,1024);      % 用于标记每个频率分量是否为噪声（1 表示是噪声，0 表示不是）
sin_window = zeros(1024, 1);
window_move_size=window_size/2;
num_fft2 = floor(total_samples / window_move_size);%总帧数
num_fft3 = floor(num_fft2/46);
%窗函数采样
for i=1:1024
...
for i=1:46
    for j=1:1024
        if sum_fft_results3(i,j,1)/sum_fft_results2(i,j,1)>0.95
