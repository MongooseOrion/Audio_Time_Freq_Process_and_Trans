...
设置较高的码矢个数可以获得更好的精确度，设置较少的码矢个数可以降低计算复杂度，在确定码矢个数时应该平衡精确度和计算复杂度，可以在实际的算法流程中确定码矢的个数。此项目设置为 16。
...
一般设置梅尔滤波器的组数在 20-24 个，此项目设置为 $M=31$ 个。设滤波器频率范围内的最低频率 $f_l = 0\text{Hz}$ ，滤波器频率范围内的最高频率 $f_h=24\text{kHz}$ （受限奈奎斯特采样定理，最高是采样频率的一半），FFT 时的长度 $N=1024$ ，采样频率 $f_s=48\text{kHz}$ ，则可将 $F_{\text{mel}}(f_l)$ 到 $F_{\text{mel}}(f_h)$ 的 Mel 频率范围均分为 $31$ 段，产生 $32$ 个 Mel 频率值，则 32 个 Mel 频率值可用下述公式计算：
