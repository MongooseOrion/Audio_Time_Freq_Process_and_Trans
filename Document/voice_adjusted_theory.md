...
一般设置梅尔滤波器的组数在 20-24 个，此项目设置为 $M=20$ 个。设滤波器频率范围内的最低频率 $f_l = 0\text{Hz}$ ，滤波器频率范围内的最高频率 $f_h=24\text{kHz}$ （受限奈奎斯特采样定理，最高是采样频率的一半），FFT 时的长度 $N=1024$ ，采样频率 $f_s=48\text{kHz}$ ，则可将 $F_{\text{mel}}(f_l)$ 到 $F_{\text{mel}}(f_h)$ 的 Mel 频率范围均分为 $m+1=21$ 段，产生 $M+2=22$ 个 Mel 频率值，则 22 个 Mel 频率值可用下述公式计算：

$$F_{mel}(i) = F_{\text{mel}}(f_l) + i\frac{F_{\text{mel}}(f_h)-F_{\text{mel}}(f_l)}{M+1},\quad i\in \mathbb{Z}, 0\le i < 22$$

则对应的实际频率值可用下述公式计算：

$$f(i) = F_{\text{mel}}^{-1}(F_{\text{mel}}(f_l) + i\frac{F_{\text{mel}}(f_h)-F_{\text{mel}}(f_l)}{M+1}),\quad i\in \mathbb{Z}, 0\le i < 22$$
...
对于此项目，一次处理得到的 MFCC 系数个数应该有 20 个。